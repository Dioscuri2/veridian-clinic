...
const insightPlaceholders = [
 { title: "Why Fasting Insulin is the Missing Link in Longevity", category: "Metabolic Health" },
 { title: "ApoB vs LDL: The Real Cardiovascular Risk Marker", category: "Cardiovascular Risk" },
 { title: "Reversing Metabolic Syndrome in 12 Weeks", category: "Clinical Protocols" },
];
...
export default function HomePage() {
 ...
 <Link href="/book?tier=baseline" className="btn btn-fo">Book Baseline Audit →</Link>
 <Link href="/assessments" className="btn btn-go">View Pricing &amp; Pathways</Link>
 </div>
 ...
 <section className="sec bg-wh"><div className="wrap"><div className="sh text-center"><p className="lbl">Latest Insights</p><div className="rule rule-c"/><h2 className="sh-title">Clinical insight, coming soon.</h2><p className="sh-body" style={{ fontSize: "1rem", maxWidth: 760 }}>We’re building a library of practical longevity and metabolic health content for patients who want signal, not noise.</p></div><div className="g3">{insightPlaceholders.map((item) => (<article key={item.title} className="card" style={{ display: "flex", flexDirection: "column", minHeight: 320 }}><span className="lbl" style={{ marginBottom: 18 }}>{item.category}</span><h3 className="cg" style={{ fontSize: "1.75rem", fontWeight: 500, color: "var(--sl)", lineHeight: 1.2, marginBottom: 16 }}>{item.title}</h3><p style={{ fontSize: ".92rem", color: "var(--sl2)", lineHeight: 1.9, marginBottom: 22, flexGrow: 1 }}>A concise, evidence-led explainer is on the way. Expect practical interpretation, biomarker context and clear clinical takeaways.</p><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 16, borderTop: "1px solid rgba(0,0,0,.07)" }}><span style={{ fontSize: ".72rem", fontWeight: 600, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--sl3)" }}>Placeholder</span><Link href="#newsletter" style={{ fontSize: ".78rem", fontWeight: 600, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--fo)" }}>Notify me when this goes live →</Link></div></article>))}</div></div></section>
 ...
 <section id="newsletter" className="sec bg-wh"><div className="wrap" style={{ maxWidth: 980 }}><div style={{ background: "linear-gradient(135deg, rgba(13,40,24,1) 0%, rgba(19,31,46,1) 100%)", padding: "clamp(28px,6vw,54px)", display: "grid", gap: 28, border: "1px solid rgba(0,0,0,.08)" }}><div><p className="lbl" style={{ color: "var(--go2)", marginBottom: 14 }}>Veridian Intelligence</p><h2 className="cg" style={{ fontSize: "clamp(2rem,4.5vw,3rem)", fontWeight: 500, color: "var(--iv)", lineHeight: 1.18, marginBottom: 14 }}>Longevity science, translated.</h2><p style={{ fontSize: "1rem", color: "rgba(246,241,232,.72)", lineHeight: 1.9, maxWidth: 680 }}>Get the latest metabolic research and clinical protocols delivered to your inbox.</p></div><div className="g2" style={{ gap: 16, alignItems: "end" }}><div><label htmlFor="newsletter-email" style={{ display: "block", fontSize: ".72rem", fontWeight: 600, letterSpacing: ".12em", textTransform: "uppercase", color: "rgba(246,241,232,.42)", marginBottom: 10 }}>Email address</label><input id="newsletter-email" type="email" placeholder="Enter your best email" className="form-field" style={{ background: "rgba(246,241,232,.96)" }} /></div><div style={{ display: "flex", flexDirection: "column", gap: 10 }}><button className="btn btn-go" style={{ width: "100%" }}>Join the Newsletter →</button><span style={{ fontSize: ".72rem", color: "rgba(246,241,232,.42)", lineHeight: 1.7 }}>Placeholder block ready for Brevo, ConvertKit or native form wiring.</span></div></div></div></div></section>
 <section className="sec bg-fo" style={{ textAlign:"center" }}><div style={{ maxWidth:680, margin:"0 auto" }}><div className="vline"/><h2 className="cg" style={{ fontSize:"clamp(2rem,4.5vw,3rem)", fontWeight:500, color:"var(--iv)", lineHeight:1.25, marginBottom:16 }}>Your biomarkers are not background noise.<br/><em style={{ fontStyle:"italic", color:"var(--go2)" }}>They are the earliest warning system you have.</em></h2><p style={{ fontSize:"1rem", color:"rgba(246,241,232,.58)", lineHeight:1.95, marginBottom:36 }}>Get clarity on what is driving decline, and a clinical plan to reverse course while it is still modifiable.</p><div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", marginBottom:36 }}><Link href="/book?tier=baseline" className="btn btn-go">Book Baseline Audit →</Link><Link href="/assessments" className="btn btn-ol-lt">View All Assessments</Link></div><div style={{ display:"flex", flexWrap:"wrap", gap:16, justifyContent:"center" }}><span style={{ fontSize:".67rem", fontWeight:600, letterSpacing:".14em", textTransform:"uppercase", color:"rgba(246,241,232,.38)" }}>CQC Registered</span><span style={{ color:"rgba(246,241,232,.2)", margin:"0 4px" }}>|</span><span style={{ fontSize:".67rem", fontWeight:600, letterSpacing:".14em", textTransform:"uppercase", color:"rgba(246,241,232,.38)" }}>GP Led</span><span style={{ color:"rgba(246,241,232,.2)", margin:"0 4px" }}>|</span><span style={{ fontSize:".67rem", fontWeight:600, letterSpacing:".14em", textTransform:"uppercase", color:"rgba(246,241,232,.38)" }}>ApoB · Insulin · Homocysteine</span></div></div></section>
 </main>
 <Footer />
 </>
 );
