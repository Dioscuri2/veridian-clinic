...
const programme = [
 ...
 "Clinical partner review where needed to assess progress and adapt treatment",
...
const faqs = [
 ...
 { q:"Is Veridian CQC registered?", a:"Veridian Clinic itself is not presented as the regulated provider. Regulated medical services are available via our CQC-registered clinical partners." },
 { q:"How is care delivered?", a:"Veridian combines longevity-focused health optimisation with regulated medical care delivered through trusted CQC-registered partner providers where needed." },
 ...
 { q:"Can I access GLP-1 therapy?", a:"Where clinically indicated, prescribing pathways may be available via our CQC-registered partner providers as part of a structured programme — never as an isolated shortcut." },
...
export default function HomePage() {
 ...
 <span className="badge"><Shield/>Clinical Partners</span>
 <span className="badge">Longevity Focused</span>
 ...
 Optimise Your Metabolic Health. Extend Your Healthspan.
 </h1>
 <p className="a3" style={{ fontSize: "clamp(1.08rem,2.7vw,1.28rem)", color: "var(--sl2)", lineHeight: 1.95, maxWidth: 650, marginBottom: 14 }}>
 A longevity-focused clinic helping you address the root causes of weight gain, fatigue, and metabolic dysfunction — not just the symptoms.
 </p>
 <p className="a3" style={{ fontSize: "1rem", color: "var(--sl3)", marginBottom: 28, fontWeight: 400, lineHeight: 1.85, maxWidth: 680 }}>
 We focus on what conventional care often overlooks — combining medical insight with advanced health optimisation strategies.
 </p>
 <p className="a3" style={{ fontSize: ".82rem", color: "var(--sl3)", marginBottom: 28, fontWeight: 500, lineHeight: 1.85, maxWidth: 680 }}>
 Regulated medical services are available via our CQC-registered clinical partners. Veridian delivers the longevity and health optimisation layer.
 </p>
 ...
 <section id="process" className="sec bg-iv"><div className="wrap"><div className="sh text-center"><p className="lbl">How Your Care Is Delivered</p><div className="rule rule-c"/><h2 className="sh-title">Integrated care, clearly structured.</h2><p className="sh-body" style={{ fontSize: "1rem", maxWidth: 760 }}>At Veridian Clinic, we combine medical care with advanced longevity-focused support so the regulated and optimisation layers are clear, intentional and joined up.</p></div><div className="gproc">{[{n:"01",t:"Clinical Assessment & Prescribing",d:"Delivered by CQC-registered medical providers through our trusted clinical partners when regulated care is needed."},{n:"02",t:"Longevity & Health Optimisation",d:"Delivered by our in-house team, focusing on lifestyle, metabolic health and long-term wellbeing."},{n:"03",t:"Integrated Approach",d:"We bridge the gap between traditional medicine and proactive health optimisation so your plan feels coherent rather than fragmented."},{n:"04",t:"Measured Progress",d:"We use biomarker review, behaviour change support and structured follow-up to keep your progress practical, trackable and sustainable."}].map(s => (<div key={s.n} style={{ padding:"28px 0", borderBottom:"1px solid rgba(0,0,0,.07)" }}><div style={{ fontSize:".62rem", fontWeight:700, letterSpacing:".28em", color:"var(--go)", marginBottom:10 }}>{s.n}</div><div style={{ width:1, height:32, background:"var(--go)", opacity:.45, marginBottom:14 }}/><h3 className="cg" style={{ fontSize:"1.65rem", fontWeight:500, color:"var(--fo)", marginBottom:8 }}>{s.t}</h3><p style={{ fontSize:".95rem", color:"var(--sl2)", lineHeight:1.9 }}>{s.d}</p></div>))}</div></div></section>
 ...
 <section id="team" className="sec bg-iv"><div className="wrap"><div className="sh text-center"><p className="lbl">Our Team</p><div className="rule rule-c"/><h2 className="sh-title">Clinical insight with longevity expertise.</h2></div><div className="g2">{[{ name:"Dr Oluwatosin Taiwo", creds:"MBBS · MRCGP · MRCS", role:"General Practitioner with a focus on metabolic health and longevity medicine", bio:"Dr Taiwo founded Veridian Clinic to bridge the gap between conventional care and more proactive metabolic and longevity thinking. His work focuses on identifying the drivers of decline early and shaping clear, high-trust pathways for long-term health optimisation.", tag:"Founder", image:"/dr-oluwatosin-taiwo.jpg" },{ name:"Tolu Taiwo", creds:"Longevity & Lifestyle Medicine Specialist", role:"Longevity & Lifestyle Medicine Specialist", bio:"Provides health optimisation coaching, metabolic guidance, and lifestyle strategy with a focus on nutrition, preventative health, and long-term wellbeing. This role is non-prescribing and focused on advisory and optimisation support.", tag:"Optimisation Lead" }].map(doc => (<div key={doc.name} className="doc-card"><div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:20 }}><div className="doc-avatar" style={{background: "#e5e7eb", color: "#6b7280", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", overflow:"hidden"}}>{doc.image ? <img src={doc.image} alt={doc.name} style={{ width:"100%", height:"100%", objectFit:"cover" }} /> : "Photo"}</div><span className="doc-tag">{doc.tag}</span></div><h3 className="doc-name">{doc.name}</h3><p className="doc-creds">{doc.creds}</p><p className="doc-role">{doc.role}</p><p className="doc-bio">{doc.bio}</p></div>))}</div><div className="cqc-strip"><div style={{ display:"flex", flexWrap:"wrap", gap:28 }}>{[["Clinical Partners","Regulated medical services available via trusted partner providers"],["Longevity Focus","Advanced metabolic and health optimisation support"],["Integrated Care","Traditional medicine aligned with proactive wellbeing strategy"]].map(([t,d]) => (<div key={t} className="cqc-item"><Shield/><div><p className="cqc-label">{t}</p><p className="cqc-sub">{d}</p></div></div>))}</div><Link href="/book?tier=baseline" className="btn btn-go" style={{ fontSize:".8rem", padding:"11px 20px" }}>Book Baseline Audit →</Link></div></div></section>
 ...
 <section className="sec bg-fo" style={{ textAlign:"center" }}><div style={{ maxWidth:760, margin:"0 auto" }}><div className="vline"/><h2 className="cg" style={{ fontSize:"clamp(2rem,4.5vw,3rem)", fontWeight:500, color:"var(--iv)", lineHeight:1.25, marginBottom:16 }}>Long-term health needs more than symptom management.<br/><em style={{ fontStyle:"italic", color:"var(--go2)" }}>It needs clarity, structure, and follow-through.</em></h2><p style={{ fontSize:"1rem", color:"rgba(246,241,232,.58)", lineHeight:1.95, marginBottom:24 }}>Get clarity on what is driving decline, and a plan that combines regulated medical pathways with meaningful health optimisation support.</p><div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", marginBottom:28 }}><Link href="/metabolic-scorecard" className="btn btn-go">Take the Metabolic Quiz →</Link><Link href="/assessments" className="btn btn-ol-lt">View All Assessments</Link></div><div style={{ background:"rgba(246,241,232,.05)", border:"1px solid rgba(246,241,232,.08)", padding:"24px", textAlign:"left", marginBottom:26 }}><LeadCaptureForm source="homepage-waitlist" title="Join the Veridian waitlist" subtitle="Get early access to new longevity pathways, flagship offers, and your next-step roadmap." ctaLabel="Join Waitlist →" buttonClassName="btn btn-go btn-full" compact /></div><div style={{ display:"flex", flexWrap:"wrap", gap:16, justifyContent:"center" }}><span style={{ fontSize:".67rem", fontWeight:600, letterSpacing:".14em", textTransform:"uppercase", color:"rgba(246,241,232,.38)" }}>Clinical Partners</span><span style={{ color:"rgba(246,241,232,.2)", margin:"0 4px" }}>|</span><span style={{ fontSize:".67rem", fontWeight:600, letterSpacing:".14em", textTransform:"uppercase", color:"rgba(246,241,232,.38)" }}>Longevity Focus</span><span style={{ color:"rgba(246,241,232,.2)", margin:"0 4px" }}>|</span><span style={{ fontSize:".67rem", fontWeight:600, letterSpacing:".14em", textTransform:"uppercase", color:"rgba(246,241,232,.38)" }}>ApoB · Insulin · Homocysteine</span></div></div></section>
 </main>
 <Footer />
 </>
 );
